const TOPIC_09 = {
    id: 9,
    title: "ធរណីមាត្រវិភាគក្នុងលំហ",
    icon: "🌐",
    iconPath: "icons/09.png",
    tag: "មេរៀនទី៩",
    semester: 1,
    description: "សមីការប៉ារ៉ាម៉ែត្រ សមីការឆ្លុះ ស្វ៊ែរ និងប្លង់ក្នុងលំហ",
    pdfs: [
        { name: "វុិចទ័រក្នុងលំហ.pdf", path: "pdfs/12_អាំងតេក្រាលកំណត់/lession/វុិចទ័រក្នុងលំហ.pdf", type: "lesson" },
        { name: "វុិចទ័រលំហាត់.pdf", path: "pdfs/12_អាំងតេក្រាលកំណត់/practies/វុិចទ័រលំហាត់.pdf", type: "exercise" }
    ],
    research: [
        { name: "vecteur-fully.pdf", path: "pdfs/12_អាំងតេក្រាលកំណត់/research/vecteur-fully.pdf" },
        { name: "កម្រងលំហាត់ជ្រើសរើសធរណីមាត្រថ្នាក់ទី_១២_អោ_មណីចន្រ្ទា.pdf", path: "pdfs/12_អាំងតេក្រាលកំណត់/research/កម្រងលំហាត់ជ្រើសរើសធរណីមាត្រថ្នាក់ទី_១២_អោ_មណីចន្រ្ទា.pdf" }
    ],
    exercises: [
        {
            id: 1,
            question: "រកប្រវែង AB ដែល A(1,2,3) និង B(4,6,8)",
            hint: "ប្រើរូបមន្ត \\(\\displaystyle d(A,B) = \\sqrt{(x_2-x_1)^2 + (y_2-y_1)^2 + (z_2-z_1)^2}\\)",
            answer: "\\(\\displaystyle d = \\sqrt{(4-1)^2 + (6-2)^2 + (8-3)^2} = \\sqrt{9+16+25} = \\sqrt{50} = 5\\sqrt{2}\\)"
        },
        {
            id: 2,
            question: "រកសមីការបន្ទាត់កាត់ចំណុច A(1,2,3) និង B(4,6,8)",
            hint: "ប្រើទម្រង់ប៉ារ៉ាម៉ែត្រ",
            answer: "\\(\\displaystyle x = 1 + 3t, \\quad y = 2 + 4t, \\quad z = 3 + 5t\\)"
        },
        {
            id: 3,
            question: "រកសមីការស្វ៊ែរដែលមានផ្ចឹត O(1,2,3) និងកាំង R = 4",
            hint: "ប្រើរូបមន្ត \\(\\displaystyle (x-a)^2 + (y-b)^2 + (z-c)^2 = R^2\\)",
            answer: "\\(\\displaystyle (x-1)^2 + (y-2)^2 + (z-3)^2 = 16\\)"
        }
    ],
    content: `
        <h3>១. ចំណុចក្នុងលំហ ℝ³</h3>
        <p>ចំណុច M(x, y, z) មានកូអរដោនេ 3 វិមាត្រ</p>

        <div class="formula-box">
            \\(\\displaystyle M(x, y, z) \\in \\mathbb{R}^3\\)
        </div>

        <h3>២. រវាងចំណុចពីរ</h3>
        <div class="formula-box">
            \\(\\displaystyle d(A,B) = \\sqrt{(x_2-x_1)^2 + (y_2-y_1)^2 + (z_2-z_1)^2}\\)
        </div>

        <h3>៣. សមីការប៉ារ៉ាម៉ែត្រ</h3>
        <p>សរសៃរ AB កាត់ចំណុច M ដែល M = (1-t)A + tB</p>

        <h4>ទម្រង់សមីការឆ្លុះ (Symmetric)</h4>
        <div class="formula-box">
            \\(\\displaystyle \\frac{x-x_1}{a} = \\frac{y-y_1}{b} = \\frac{z-z_1}{c}\\)
        </div>

        <h4>ទម្រង់ប៉ារ៉ាម៉ែត្រ</h4>
        <div class="formula-box">
            \\(\\displaystyle x = x_1 + at, \\quad y = y_1 + bt, \\quad z = z_1 + ct\\)
        </div>

        <h3>៤. សមីការប្លង់</h3>
        <div class="formula-box">
            \\(\\displaystyle ax + by + cz + d = 0\\)
        </div>

        <h3>៥. សមីការស្វ៊ែរ</h3>
        <div class="formula-box">
            \\(\\displaystyle (x-a)^2 + (y-b)^2 + (z-c)^2 = R^2\\)
        </div>

        <p>ដើម្បីមើលរូបស្វ៊ែរក្នុងលំហ៖</p>
        <div class="geogebra-embed">
            <iframe src="https://www.geogebra.org/calculator/3d?embed" width="100%" height="500px" style="border:0;border-radius:12px;" allowfullscreen></iframe>
        </div>
        <p style="text-align:center;margin-top:0.5rem;">
            <a href="https://www.geogebra.org/calculator/3d" target="_blank" class="pdf-btn pdf-btn-view" style="display:inline-flex;">
                បើក GeoGebra 3D ក្នុង Tab ថ្មី
            </a>
        </p>

        <div class="example-box">
            <h4>ឧទាហរណ៍៖</h4>
            <p>រកប្រវែង AB ដែល A(1,2,3) និង B(4,6,8)</p>
            <p>\\(\\displaystyle d = \\sqrt{(4-1)^2 + (6-2)^2 + (8-3)^2} = \\sqrt{9+16+25} = \\sqrt{50} = 5\\sqrt{2}\\)</p>
        </div>
    `
};
